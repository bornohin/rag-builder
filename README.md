...
**This repository is the workspace.** Clone it *into* the project you want to index,
as a directory named `rag-workspace`, and it becomes that project's retrieval server.
...
The workspace indexes its **parent** directory, so a layout of several sibling
checkouts under one folder is indexed as one searchable corpus.
...
```json
{
  "mcpServers": {
    "codebase-rag": {
      "type": "stdio",
      "command": "<project>/rag-workspace/.poc-venv/bin/python3",
      "args": ["<project>/rag-workspace/mcp_server.py"],
      "env": {}
    }
  }
}
